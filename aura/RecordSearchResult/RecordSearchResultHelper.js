({
    ...
    initFlows: function (c, h, field) {
        const flowNames = c.get("v.flowNames")
            ? c.get("v.flowNames").split(",")
            : [];
        const flowLabels = c.get("v.flowLabels")
            ? c.get("v.flowLabels").split(",")
            : [];
        ...
        h.getRecords(
            ...
            .then(
                $A.getCallback(function (records) {
                    ...
                    console.log("★★ getRecords .then3");
                })
            )
            .catch(function (reason) {
                h.showError(c, h, "controller.loadData : " + reason);
                console.log("★★ getRecords .catch4");
            })
            .then(
                $A.getCallback(function () {
                    c.find("dataTable").set("v.isLoading", false);
                    console.log("★★ getRecords .then5");
                })
            );
        console.log("★★★★ 関数を終了する ： loadData");
    },
    createColumn: function (c, h, field) {
        console.log("★★★★ 関数を開始する ： createColumn");
    ...
    navigateToSObject: function (c, h, recordId) {
        console.log("★★★★ 関数を開始する ： navigateToSObject");
        ...
        console.log("★★★★ 関数を終了する ： navigateToSObject");
    },
    ...
    getRecords: function (c, h, objectName, fieldsJson, recordIdsJson) {
        console.log("★★★★ 関数を開始する ： getRecords");
        console.log("★★ 変数 objectName ： ", objectName);
        console.log("★★ 変数 fieldsJson ： ", fieldsJson);
        console.log("★★ 変数 recordIdsJson ： ", recordIdsJson);
        ...
        return new Promise(function (resolve, reject) {
            ...
            $A.enqueueAction(action);
            console.log("★★★★ 関数を終了する ： getRecords");
        });
    },
